...
import ProcessSelect from '../proccess/components/processSelect';
import ProcessStepSelect from '../proccess/components/processStepSelect';
...
class JobCreateBox extends Component {
    constructor(props) {
        super(props);
        // const stt = this.props.jobStatus.filter(s => !s.isCancel && !s.isComplated)[0];
        const stt = this.props.jobStatus.items.filter(s => !s.isCancelStatus && !s.isCompleteStatus)[0];
        ...
        this.state = {
            entry: {
                ...
                processStepId: this.props.stepId,
            },
            changed: false,
            // loading: false
        }
    ...
    save = () => {
        ...
        this.props.actions.create(this.state.entry)
            .then(data => {
                this.props.onRequestClose()
                // Toast.show("Thêm mới công việc thành công");
                Toast.show({
            ...
            .catch(({ error, message }) => {
                this.props.onRequestClose()
                alert("Thêm mới công việc thất bại", message);
            });
        // this.props.onRequestClose();
    }
    ...
    render() {
        ...
        return (
            <Modal
                ...
                <Toolbar
                    style={{ backgroundColor: entry.statusName.color }}
                    ...
                    <ScrollView keyboardShouldPersistTaps='always'>
                        <View style={styles.jobInfo}>
                            ...
                            <View style={styles.jobProp}>
                                ...
                                //     this.setValue({ customers: items.map(item => ({ id: item.id })) })}
                                // selectedValue={entry.customers.map(item => item.id)}
                                />
                            </View>
                            ...
                            <View style={styles.jobProp}>
                                ...
                                <ProcessSelect style={styles.select}
                                    onValueChange={text => this.setValue({ processId: text || 0 })}
                                    selectedValue={this.state.entry.processId}
                                />
                            </View>

                            <View style={styles.jobProp}>
                                <Text style={styles.label}>{__('Bước chiến dịch')}</Text>
                                <ProcessStepSelect style={styles.select}
                                    processId={this.state.entry.processId}
                                    onValueChange={text => this.setValue({ processStepId: text || 0 })}
                                    selectedValue={this.state.entry.processStepId}
                                />
                            </View>
                            ...
                            <View style={styles.jobProp}>
                                ...
                                // onValueChange={(value, item) => this.setManager(item)}
                            ...
                            <View style={styles.jobProp}>
                                ...
                                // onValueChange={(values, items) => this.setUsers(items)}                                  
                                // selectedValue={userIds} 
                                />
                            </View>
...
export default connect(JobCreateBox, state => ({
    job: state.job,
