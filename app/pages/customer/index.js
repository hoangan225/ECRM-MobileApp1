...
import ActionSheet from './primary/customerBoxActionSheet';
import CustomerBox from './primary/customerPartBox';
import CustomerDataFilter from './primary/customerPartDataFilter';
import CustomerSearchBox from './primary/customerPartSearch';
import CustomerViewFilter from './primary/customerSortDisplay';
...
class Customer extends Component {
    ...
    componentDidUpdate(preProps) {
        if (preProps.customer.loaded && !this.props.customer.loaded) {
    ...
    applySearch = (data, isTab) => {
        ...
        if (isTab && this.canList) {
            var view = this.props.customer.views.filter(t => t.viewId == data.viewId);
            if ((!view) || view.length == 0) {
                // console.log("view aplysearch")
                this.setState({ loading: true });
                this.props.actions.getlist({ ...search })
                    .then(() => {
                        this.setState({ loading: false });
            ...
            if (data.filters) {
                ...
                // console.log('applysearch cus nav');
    ...
    handleChangeTab = (index) => {
        ...
        // this.arrTab.push(index);
        // this.arrTab = Array.from(new Set(this.arrTab.map(item => item)));

        // var isView = this.arrTab.findIndex(k => k == index);
        // // console.log(isView, "íview");
        // if (isView < 0) {
        ...
        if (views && views.length > 0) {
            ...
            this.arrTab.push(viewId);
            this.arrTab = Array.from(new Set(this.arrTab.map(item => item)));

            var isView = this.arrTab.findIndex(k => k == this.state.search.viewId);
            // // console.log(isView);
            // if (isView < 0) {
            ...
            this.setState({
                ...
                // // console.log(views[index].id, "views[index].id");
                // this.applySearch({ viewId: views[index].id }, true)
                var view = this.props.customer.views.filter(t => t.viewId == views[index].id);
                var param = { ...this.state.search, viewId: views[index].id };
                if ((!view) || view.length == 0) {
                    this.props.actions.getlist({ ...param })
                        .catch(({ error, message }) => {
                            Toast.show(message, 'danger');
                        });
                }
        ...
        if ((!lstCustomer) || (lstCustomer.lenght == 0)) {
            this.updateView();
            // console.log("this.updateView() tab")
        }
        // }
        // else {
        //     // console.log("tab load finish");
        //     this.setState({
        //         currentViewIndex: index
        //     });
        // }
    ...
    render() {
        // // console.log('this.arraytab', this.arrTab);
        // // console.log(this.props.customer.viewIds, "viewIds");
        ...
        this.currentView = this.sortView()[this.state.currentViewIndex];
        ...
        return (
            <Container style={styles.container}>
                ...
                <CustomerDataFilter
                    ...
                    viewId={this.state.search.sort.viewId}
                    title={'Khách hàng'}
    ...
    renderView = ({ route }) => {
        ...
        if (customer && customer.views.length > 0) {
            ...
            // // console.log(customer, "íview111111");
